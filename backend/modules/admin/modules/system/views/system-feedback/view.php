<?php

use yii\helpers\Html;
use yii\widgets\DetailView;
use common\helpers\Models;

$this->title = '礼物详情-' . Yii::$app->setting->get('siteName');
$this->registerCssFile(yii\helpers\Url::to('/style/css/my_shop.css'), ['depends' => ['backend\assets\SystemAsset']]);
?>
<div class="x_section_all">
    <div class="x_section">
        <div class="x_section_left">
            <?= backend\modules\admin\widgets\AdminSideNav::widget(["id" => $master_navigation_id]); ?>
        </div>
        <!-- 产品管理 -->
        <div class="x_section_r">

            <div class="x_prompt" style="margin-top:15px;border-bottom:none">
                <h2 style="margin-top:5px;">礼物详情</h2>	
            </div>
           <?= backend\modules\admin\widgets\AlertWidget::widget();?> 
            <p style="float: right">
                <?= Html::a('修改', ['create', 'id' => $model->id], ['class' => 'btn btn-primary']) ?>
                <?= Html::a('删除', ['delete', 'id' => $model->id], ['class' => 'btn btn-danger', 'data' => ['confirm' => '确定删除吗?','method' => 'get']]) ?>
                <?= Html::a('返回', ['index'], ['class' => 'btn btn-default',]) ?>
            </p>
            <?= DetailView::widget([
                'model' => $model,
                'attributes' => [
                    'id',
                    //'user_id',
                    [
                        'attribute' => 'user_id',
                        'format' => ['raw'],
                        'value'  => isset($model->user) && !empty($model->user) ? $model->user->getUserName() : "",
                    ],                    
                    'version',
                    //'system',
                    [
                        'attribute' => 'system',
                        'format' => ['raw'],
                        'value'  => $model->getSystem($model->system),
                    ],
                    [
                        'attribute' => 'contact_type',
                        'format' => ['raw'],
                        'value'  => $model->getContactType($model->contact_type),
                    ],                    
                    'contact',
                    'content',
                    [
                        'attribute' => 'handle_status',
                        'format' => ['raw'],
                        'value'  => $model->getHandleStatus($model->handle_status),
                    ],
                    'handle_remark',
                    'created_at',
                    'created_uid',
                    'updated_at',
                    'updated_uid',
                ],
            ]) ?>   
            <p style="float: right">
                <?= Html::a('修改', ['create', 'id' => $model->id], ['class' => 'btn btn-primary']) ?>
                <?= Html::a('删除', ['delete', 'id' => $model->id], ['class' => 'btn btn-danger', 'data' => ['confirm' => '确定删除吗?','method' => 'get']]) ?>
                <?= Html::a('返回', ['index'], ['class' => 'btn btn-default']) ?>
            </p>   
        </div>
        <!-- 产品管理 end-->
    </div>
</div>


